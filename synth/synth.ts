...
//	                    = CharCode.K,
    ...
    additive	        = CharCode.NUM_0, // added in Slarmoo's box URL version 2, currently unused
//                      = CharCode.NUM_1,
//                      = CharCode.NUM_2, 
//                      = CharCode.NUM_3, 
//          	        = CharCode.NUM_4, 
    ...
    public fromJsonObject(patternObject: any, song: Song, channel: Channel, importedPartsPerBeat: number, isNoiseChannel: boolean, isModChannel: boolean, jsonFormat: string = "auto"): void {
        ...
        if (patternObject["notes"] && patternObject["notes"].length > 0) {
            ...
            for (let j: number = 0; j < patternObject["notes"].length; j++) {
                ...
                if ((format != "ultrabox" && format != "slarmoosbox" && format != "ouroboros") && instrument.modulators[mod] == Config.modulators.dictionary["tempo"].index) {
                    for (const pin of note.pins) {
...
export class AdditiveWave {
    ...
    public reset(): void {
        for (let i: number = 0; i < Config.additiveControlPoints; i++) {
            this.additives[i] = 0;
            this.waveTypes[i] = BaseWaveTypes.triangle;
        }
    ...
    public markCustomWaveDirty(): void {
        const hashMult: number = Synth.fittingPowerOfTwo(Config.additiveMax + 2) - 1;
        let hash: number = 0;
        for (const point of this.additives) hash = ((hash * hashMult) + point) >>> 0;
        this.hash = hash;
    }
}
...
class AdditiveWaveState {
    ...
    public getCustomWave(settings: AdditiveWave): Float32Array {
        if (this._hash == settings.hash) return this.wave!;
        this._hash = settings.hash;

        const additiveRendered: number = Config.additiveRendered;

        const waveLength: number = Config.additiveWavelength;
    ...
    private waveExpressions(waveType: number, time: number, harmonic: number): number {
        switch (waveType) {
            ...
            //     return this.waveExpressions(BaseWaveTypes.triangle, time, harmonic) + this.waveExpressions(BaseWaveTypes.triangle, (time + 1 / 2), harmonic);
            default:
...
export class Instrument {
    ...
    public readonly additiveWave: AdditiveWave = new AdditiveWave();
    ...
    public fromJsonObject(instrumentObject: any, isNoiseChannel: boolean, isModChannel: boolean, useSlowerRhythm: boolean, useFastTwoNoteArp: boolean, legacyGlobalReverb: number = 0, jsonFormat: string = Config.jsonFormat): void {
        ...
        if (instrumentObject["volume"] != undefined) {
            if (format == "jummbox" || format == "midbox" || format == "synthbox" || format == "goldbox" || format == "paandorasbox" || format == "ultrabox" || format == "slarmoosbox" || format == "ouroboros") {
        ...
        if (instrumentObject["additives"] != undefined) {
            for (let i: number = 0; i < Config.additiveControlPoints; i++) {
                ...
                if (instrumentObject["additiveWaves"][i] != undefined) {
                    ...
                    this.additiveWave.waveTypes[i] = BaseWaveTypes.sine;
                }
            }
        } else {
...
export class Song {
    ...
    private static readonly _oldestOuroborosVersion: number = 1;
    ...
    public fromBase64String(compressed: string, jsonFormat: string = "auto"): void {
        ...
        let fromOuroboros: boolean = false;
        ...
        if (fromOuroboros && (version == -1 || version > Song._latestOuroborosVersion || version < Song._oldestOuroborosVersion)) return;
        ...
        if (fromUltraBox || fromGoldBox || fromAbyssBox || fromSlarmoosBox || fromOuroboros) {
        ...
        while (charIndex < compressed.length) switch (command = compressed.charCodeAt(charIndex++)) {
            case SongTagCode.songDetails: {
                ...
                if ((fromUltraBox && !beforeSix)||(fromAbyssBox && !beforeThree)||fromOuroboros) {
            ...
            case SongTagCode.rhythm: {
                if (!fromUltraBox && !fromAbyssBox && !fromSlarmoosBox && !fromOuroboros) {
		...
		//rhythm fixes
            ...
            case SongTagCode.preset: {
                ...
                // Similar story, supersaw is also before custom chip (and mod, but mods can't have presets).
                    ...
                    if (this.channels[instrumentChannelIterator].instruments[instrumentIndexIterator].preset == InstrumentType.mod) {
                        this.channels[instrumentChannelIterator].instruments[instrumentIndexIterator].preset = InstrumentType.additive;
                        this.channels[instrumentChannelIterator].instruments[instrumentIndexIterator].type = InstrumentType.additive;
                    }
                }
            ...
            case SongTagCode.eqFilter: {
                if ((beforeNine && fromBeepBox) || (beforeFive && fromJummBox) || (beforeFour && fromGoldBox)) {
                    ...
                    if (fromBeepBox || typeCheck == 0) {
                        ...
                        if (fromJummBox || fromGoldBox || fromUltraBox || fromAbyssBox || fromSlarmoosBox || fromOuroboros)
                        ...
                        if ((fromJummBox && !beforeFive) || (fromGoldBox && !beforeFour) || fromUltraBox || fromAbyssBox || fromSlarmoosBox || fromOuroboros) {
            ...
            case SongTagCode.drumsetEnvelopes: {
                ...
                const pregoldToEnvelope: number[] = [0, 1, 2, 4, 5, 6, 8, 9, 10, 12, 13, 14, 16, 17, 18, 19, 20, 21, 23, 24, 25, 27, 28, 29, 32, 33, 34, 31, 11];
                if ((beforeNine && fromBeepBox) || (beforeFive && fromJummBox) || (beforeFour && fromGoldBox)) {
                    if((beforeTwo && fromGoldBox) || (!fromGoldBox && !fromUltraBox && !fromAbyssBox && !fromSlarmoosBox && !fromOuroboros)){

                    }
                    if (instrument.type == InstrumentType.drumset) {
                        for (let i: number = 0; i < Config.drumCount; i++) {
                            let aa: number = base64CharCodeToInt[compressed.charCodeAt(charIndex++)];
                            if((beforeTwo && fromGoldBox) || (!fromGoldBox && !fromUltraBox && !fromAbyssBox && !fromSlarmoosBox && !fromOuroboros)) aa = pregoldToEnvelope[aa];
            ...
            case SongTagCode.pulseWidth: {
                const instrument: Instrument = this.channels[instrumentChannelIterator].instruments[instrumentIndexIterator];
                if ((fromAbyssBox && !beforeThree)||fromOuroboros) {
                ...
                if ((beforeNine && fromBeepBox) || (beforeFive && fromJummBox) || (beforeFour && fromGoldBox)) {
                    ...
                    let aa: number = base64CharCodeToInt[compressed.charCodeAt(charIndex++)];
                    if((beforeTwo && fromGoldBox) || (!fromGoldBox && !fromUltraBox && !fromAbyssBox && !fromSlarmoosBox && !fromOuroboros)) aa = pregoldToEnvelope[aa];
                    if ((!fromSlarmoosBox && aa >= 2)||(!fromOuroboros && aa >= 2)) aa++; //2 for pitch
            ...
            case SongTagCode.fadeInOut: {
                if ((beforeNine && fromBeepBox) || ((fromJummBox && beforeFive) || (beforeFour && fromGoldBox))) {
                    ...
                    } else if ((beforeFour && !fromGoldBox && !fromUltraBox && !fromAbyssBox && !fromSlarmoosBox && !fromOuroboros) || fromBeepBox) {
                        const settings = legacySettings[clamp(0, legacySettings.length, base64CharCodeToInt[compressed.charCodeAt(charIndex++)])];
                        const instrument: Instrument = this.channels[instrumentChannelIterator].instruments[instrumentIndexIterator];
                        instrument.fadeIn = Synth.secondsToFadeInSetting(settings.fadeInSeconds);
                        instrument.fadeOut = Synth.ticksToFadeOutSetting(settings.fadeOutTicks);
                ...
                if ((beforeNine && fromBeepBox) || ((fromJummBox && beforeFive) || (beforeFour && fromGoldBox))) {
                    ...
                    if (effectsIncludeNoteFilter(instrument.effects)) {
                        ...
                        if (fromBeepBox || typeCheck == 0) {
                            ...
                            if ((fromJummBox && !beforeFive) || (fromGoldBox) || (fromUltraBox)|| (fromAbyssBox) || fromSlarmoosBox || fromOuroboros) {
                                let usingSubFilterBitfield: number = (base64CharCodeToInt[compressed.charCodeAt(charIndex++)] << 6) | (base64CharCodeToInt[compressed.charCodeAt(charIndex++)]);
                                for (let j: number = 0; j < Config.filterMorphCount - 1; j++) {
                                    if (usingSubFilterBitfield & (1 << j)) {
                                        // Number of control points
                    ...
                    if (effectsIncludeChord(instrument.effects)) {
                        ...
                        if (Config.chords[instrument.chord].arpeggiates == true && (fromJummBox||fromGoldBox||fromUltraBox || fromAbyssBox)) {
                            instrument.arpeggioSpeed = base64CharCodeToInt[compressed.charCodeAt(charIndex++)];
                            instrument.fastTwoNoteArp = (base64CharCodeToInt[compressed.charCodeAt(charIndex++)]) ? true : false;
                        } else if (Config.chords[instrument.chord].strumParts > 0 && ((fromUltraBox && !beforeSix)||(fromAbyssBox && !beforeThree)||fromOuroboros)) {
                    ...
                    if (effectsIncludeVibrato(instrument.effects)) {
                        ...
                        // Custom vibrato
                        if (instrument.vibrato == Config.vibratos.length && (fromJummBox || fromGoldBox || fromUltraBox || fromAbyssBox || fromSlarmoosBox || fromOuroboros)) {
                    ...
                    if (effectsIncludePanning(instrument.effects)) {
                        ...
                        if ((fromJummBox && !beforeTwo) || fromGoldBox || fromUltraBox || fromAbyssBox || fromSlarmoosBox || fromOuroboros)
            ...
            case SongTagCode.pan: {
                if (beforeNine && fromBeepBox) {
                    ...
                    if (fromJummBox && !beforeThree || fromGoldBox || fromUltraBox || fromAbyssBox || fromSlarmoosBox || fromOuroboros) {
                        instrument.panDelay = base64CharCodeToInt[compressed.charCodeAt(charIndex++)];
                    }
                } else {
                    // Do nothing? This song tag code is deprecated for now.
            ...
            case SongTagCode.channelNames: {
                for (let channel: number = 0; channel < this.getChannelCount(); channel++) {
                    ...
                    if (beforeFour && !fromGoldBox && !fromUltraBox && !fromAbyssBox && !fromSlarmoosBox && !fromOuroboros)
            ...
            case SongTagCode.operatorFrequencies: {
                const instrument = this.channels[instrumentChannelIterator].instruments[instrumentIndexIterator];
							...
							else if (!fromGoldBox && !fromUltraBox && !fromAbyssBox && !fromSlarmoosBox && !fromOuroboros) {
            ...
            case SongTagCode.envelopes: {
                ...
                if ((beforeNine && fromBeepBox) || (beforeFive && fromJummBox) || (beforeFour && fromGoldBox)) {
                    ...
                    if ((fromJummBox && !beforeSix) || (fromUltraBox && !beforeFive)|| (fromAbyssBox) || fromSlarmoosBox || fromOuroboros) {
                            ...
                            if (!useOldShape && !newNote) {
                                ...
                                if (note.start == 0) {
                                    if (!((beforeNine && fromBeepBox) || (beforeFive && fromJummBox)||(beforeFour&&fromGoldBox))) {
                                        ...
                                        if ((beforeFour && !fromUltraBox && !fromAbyssBox) || fromBeepBox || fromSlarmoosBox || fromOuroboros) {
                                            note.continuesLastPattern = false;
                                        } else {
    ...
    public toJsonObject(enableIntro: boolean = true, loopCount: number = 1, enableOutro: boolean = true): Object {
        ...
        const result: any = {
            ...
            "format": Song._format,
            "version": Song._latestOuroborosVersion,
            "author": this.author,
...
class EnvelopeComputer {
    ...
    // Fade envelopes notably use unscaled values instead of being ties to Envelope Speed.
    ...
    public computeEnvelopes(instrument: Instrument, currentPart: number, tickTimeStart: number[], tickTimeStartReal: number, secondsPerTick: number, tone: Tone | null, timeScale: number[], instrumentState: InstrumentState, synth: Synth): void {
        ...
        for (let envelopeIndex: number = 0; envelopeIndex <= instrument.envelopeCount; envelopeIndex++) {
            ...
            if (automationTarget.computeIndex != null) {
                ...
                let envelopeStart: number = EnvelopeComputer.computeEnvelope(envelope, envelopeSpeed, noteSecondsStartUnscaled, noteSecondsStart[envelopeIndex], beatTimeStart[envelopeIndex], timeSinceStart, noteSizeStart, pitch, inverse, perEnvelopeLowerBound, perEnvelopeUpperBound, false, steps, seed, waveform, defaultPitch);
                if (prevSlideStart) {
                    const other: number = EnvelopeComputer.computeEnvelope(envelope, envelopeSpeed, prevNoteSecondsStartUnscaled, prevNoteSecondsStart[envelopeIndex], beatTimeStart[envelopeIndex], timeSinceStart, prevNoteSize, pitch, inverse, perEnvelopeLowerBound, perEnvelopeUpperBound, false, steps, seed, waveform, defaultPitch);
                    envelopeStart += (other - envelopeStart) * prevSlideRatioStart;
                }
                if (nextSlideStart) {
                    const other: number = EnvelopeComputer.computeEnvelope(envelope, envelopeSpeed, 0.0, 0.0, beatTimeStart[envelopeIndex], timeSinceStart, nextNoteSize, pitch, inverse, perEnvelopeLowerBound, perEnvelopeUpperBound, false, steps, seed, waveform, defaultPitch);
                    envelopeStart += (other - envelopeStart) * nextSlideRatioStart;
                }
                let envelopeEnd: number = envelopeStart;
                if (instrument.discreteEnvelope == false) {
                    envelopeEnd = EnvelopeComputer.computeEnvelope(envelope, envelopeSpeed, noteSecondsEndUnscaled, noteSecondsEnd[envelopeIndex], beatTimeEnd[envelopeIndex], timeSinceStart, noteSizeEnd, pitch, inverse, perEnvelopeLowerBound, perEnvelopeUpperBound, false, steps, seed, waveform, defaultPitch);
                    if (prevSlideEnd) {
                        const other: number = EnvelopeComputer.computeEnvelope(envelope, envelopeSpeed, prevNoteSecondsEndUnscaled, prevNoteSecondsEnd[envelopeIndex], beatTimeEnd[envelopeIndex], timeSinceStart, prevNoteSize, pitch, inverse, perEnvelopeLowerBound, perEnvelopeUpperBound, false, steps, seed, waveform, defaultPitch);
                        envelopeEnd += (other - envelopeEnd) * prevSlideRatioEnd;
                    }
                    if (nextSlideEnd) {
                        const other: number = EnvelopeComputer.computeEnvelope(envelope, envelopeSpeed, 0.0, 0.0, beatTimeEnd[envelopeIndex], timeSinceStart, nextNoteSize, pitch, inverse, perEnvelopeLowerBound, perEnvelopeUpperBound, false, steps, seed, waveform, defaultPitch);
    ...
    public static computeEnvelope(envelope: Envelope, perEnvelopeSpeed: number, unspedTime: number, time: number, beats: number, timeSinceStart: number, noteSize: number, pitch: number, inverse: boolean, perEnvelopeLowerBound: number, perEnvelopeUpperBound: number, isDrumset: boolean = false, steps: number, seed: number, waveform: number, defaultPitch: number): number {
    ...
    public computeDrumsetEnvelopes(instrument: Instrument, drumsetFilterEnvelope: Envelope, beatsPerPart: number, partTimeStart: number, partTimeEnd: number) {
        ...
        let drumsetFilterEnvelopeStart: number = EnvelopeComputer.computeEnvelope(drumsetFilterEnvelope, 1, this.noteSecondsStartUnscaled, this.noteSecondsStartUnscaled, beatsPerPart * partTimeStart, 0, this.noteSizeStart, pitch, false, 0, 1, true, 2, 2, BaseWaveTypes.sine, pitch); //doesn't have/need pitchStart, pitchEnd, pitchInvert, steps, seed, or timeSinceBeginning

        // Apply slide interpolation to drumset envelope.
        if (this.prevSlideStart) {
            const other: number = EnvelopeComputer.computeEnvelope(drumsetFilterEnvelope, 1, this.prevNoteSecondsStartUnscaled, this.prevNoteSecondsStartUnscaled, beatsPerPart * partTimeStart, 0, this.prevNoteSize, pitch, false, 0, 1, true, 2, 2, BaseWaveTypes.sine, pitch);
            drumsetFilterEnvelopeStart += (other - drumsetFilterEnvelopeStart) * this.prevSlideRatioStart;
        }
        if (this.nextSlideStart) {
            const other: number = EnvelopeComputer.computeEnvelope(drumsetFilterEnvelope, 1, 0.0, 0.0, beatsPerPart * partTimeStart, 0, this.nextNoteSize, pitch, false, 0, 1, true, 2, 2, BaseWaveTypes.sine, pitch);
        ...
        if (instrument.discreteEnvelope == false) {
            drumsetFilterEnvelopeEnd = EnvelopeComputer.computeEnvelope(drumsetFilterEnvelope, 1, this.noteSecondsEndUnscaled, this.noteSecondsEndUnscaled, beatsPerPart * partTimeEnd, 0, this.noteSizeEnd, pitch, false, 0, 1, true, 2, 2, BaseWaveTypes.sine, pitch);

            if (this.prevSlideEnd) {
                const other: number = EnvelopeComputer.computeEnvelope(drumsetFilterEnvelope, 1, this.prevNoteSecondsEndUnscaled, this.prevNoteSecondsEndUnscaled, beatsPerPart * partTimeEnd, 0, this.prevNoteSize, pitch, false, 0, 1, true, 2, 2, BaseWaveTypes.sine, pitch);
                drumsetFilterEnvelopeEnd += (other - drumsetFilterEnvelopeEnd) * this.prevSlideRatioEnd;
            }
            if (this.nextSlideEnd) {
                const other: number = EnvelopeComputer.computeEnvelope(drumsetFilterEnvelope, 1, 0.0, 0.0, beatsPerPart * partTimeEnd, 0, this.nextNoteSize, pitch, false, 0, 1, true, 2, 2, BaseWaveTypes.sine, pitch);
...
class InstrumentState {
    ...
    public readonly additiveWave: AdditiveWaveState = new AdditiveWaveState();
...
// advloop addition
    ...
    private static additiveSynth(synth: Synth, bufferIndex: number, roundedSamplesPerTick: number, tone: Tone, instrumentState: InstrumentState): void {
            //Essentially identical to harmonics, just without unison (for now)
            const data: Float32Array = synth.tempMonoInstrumentSampleBuffer!;
            const wave: Float32Array = instrumentState.wave!;
            const waveLength: number = wave.length - 1; // The first sample is duplicated at the end, don't double-count it.

            if (!instrumentState.chord!.customInterval) tone.phases[1] = tone.phases[0];
            let phaseDeltaA: number = tone.phaseDeltas[0] * waveLength;
            const phaseDeltaScaleA: number = +tone.phaseDeltaScales[0];
            let expression: number = +tone.expression;
            const expressionDelta: number = +tone.expressionDelta;
            ...
            for (let sampleIndex: number = bufferIndex; sampleIndex < stopIndex; sampleIndex++) {
                ...
                const inputSample: number = waveA;
                const sample: number = applyFilters(inputSample, initialFilterInput1, initialFilterInput2, filterCount, filters);
                initialFilterInput2 = initialFilterInput1;
                initialFilterInput1 = inputSample;
